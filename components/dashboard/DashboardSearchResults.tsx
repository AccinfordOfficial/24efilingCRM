  ...
  return (
      <div className="space-y-6 animate-in fade-in duration-500">
          <header className="flex items-center justify-between">
              <div>
                    <h1 className="text-3xl font-bold tracking-tight dark:text-white">Search Results</h1>
                    <p className="dark:text-slate-400 mt-1">Found {totalMatches} matches for "{searchTerm}"</p>
              </div>
          ...
          {results.customers.length > 0 && (
              <div className="space-y-4">
                    <h2 className="text-xl font-semibold dark:text-white border-b border-white/10 pb-2">Customers ({results.customers.length})</h2>
                          ...
                          <Card key={c.id} className="glass-card border border-white/10 hover:border-primary/30 cursor-pointer transition-colors" onClick={() => onViewCustomer(c.id)}>
                              <CardHeader className="pb-2">
                                    <CardTitle className="flex justify-between dark:text-white">
                                        {c.name}
                                        <span className="text-xs font-normal bg-white/5 dark:text-slate-400 border border-white/10 px-2 py-1 rounded-full">
                                            {c.service_name}
                                        </span>
                                    </CardTitle>
                                    <CardDescription className="dark:text-slate-400">{c.business_name}</CardDescription>
                              </CardHeader>
                              <CardContent>
                                <div className="text-sm dark:text-slate-400 space-y-1">
          ...
          {results.leads.length > 0 && (
              <div className="space-y-4">
                    <h2 className="text-xl font-semibold dark:text-white border-b border-white/10 pb-2">Leads ({results.leads.length})</h2>
                  <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                      {results.leads.map(l => (
                          <Card key={l.id} className="glass-card border border-white/10 hover:border-primary/30 cursor-pointer transition-colors" onClick={() => onViewLead && onViewLead(l.id)}>
                              <CardHeader className="pb-2">
                                    ...
                                    <CardDescription className="dark:text-slate-400">{l.business_name}</CardDescription>
                              </CardHeader>
                              <CardContent>
                                <div className="text-sm dark:text-slate-400 space-y-1">
