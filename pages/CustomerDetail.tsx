...
const CustomerDetail: React.FC<CustomerDetailProps> = ({ customer, onBack, leads, onAddActivityToLead, refreshData, onUpdateCustomer }) => {
    ...
    const getCustomerCompleteness = (c: Customer) => {
        const fields = [
            { name: 'Customer Name', filled: !!c.name },
            { name: 'Business Name', filled: !!c.business_name },
            { name: 'Email Address', filled: !!c.email },
            { name: 'Phone Number', filled: !!c.phone },
            { name: 'PAN Number', filled: !!c.pan_number },
            { name: 'Aadhaar Card', filled: !!c.aadhar_number },
            { name: 'Residential Address', filled: !!(c.residential_address || c.personal_city) },
            { name: 'Business Address', filled: !!(c.business_address || c.business_city) },
            { name: 'Primary Service', filled: !!c.service_name },
            { name: 'Enrolled Date', filled: !!c.date_of_enroll },
        ];
    ...
    const customerCompleteness = getCustomerCompleteness(customer);
            ...
            <header>
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
                    <div className="flex items-center gap-4">
                        <div className="relative h-24 w-24 rounded-full bg-slate-200 dark:bg-slate-800 flex items-center justify-center flex-shrink-0">
                            {customer.avatar_url ? (
                        ...
                        <div>
                            <h1 className="text-3xl font-bold tracking-tight text-slate-900 dark:text-white">{customer.name}</h1>
                            <p className="text-slate-500 dark:text-slate-400 text-lg">{customer.business_name}</p>
                            <p className="text-sm text-slate-500 dark:text-slate-400">Customer since {new Date(customer.date_of_enroll).toLocaleDateString()}</p>
                        </div>
                    </div>
                    <div className="flex flex-wrap items-center gap-3">
                        <CompletenessMeter percentage={customerCompleteness.percentage} missingFields={customerCompleteness.missingFields} title="Customer Profile" />
                        {onUpdateCustomer && (
                            <Button variant="outline" size="sm" onClick={handleOpenEdit} className="bg-background dark:bg-slate-900 shadow-sm border-slate-200 dark:border-white/10 text-foreground dark:text-white hover:bg-slate-50 dark:hover:bg-slate-800 font-semibold gap-1.5 transition-all">
                                ⚙️ Edit Profile
                            </Button>
                        )}
                    </div>
                ...
                <div className="lg:col-span-2 space-y-6">
                            ...
                            {customer.service_sets && customer.service_sets.length > 0 ? (
                                <ul className="space-y-3">
                                    {customer.service_sets.map((set, idx) => (
                                        <li key={set.id || idx} className="p-4 border border-slate-200 dark:border-white/5 rounded-lg bg-slate-50 dark:bg-slate-950/40 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                                            <div>
                                                <h4 className="font-semibold text-slate-800 dark:text-slate-200">{set.mainService}</h4>
                                                <div className="flex flex-wrap gap-2 mt-1">
                                                    {set.subservices.map((sub, sIdx) => (
                                                        <span key={sIdx} className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-300">
                                ...
                                <div className="text-center py-6 text-slate-500 dark:text-slate-400 bg-slate-50 dark:bg-slate-950/40 rounded-lg border border-dashed border-slate-300 dark:border-white/10">
                    ...
                    <Card>
                        ...
                        <CardContent className="grid sm:grid-cols-2 gap-4 text-sm">
                            <div><strong className="font-medium text-slate-600 dark:text-slate-400 block">Email</strong> <a href={`mailto:${customer.email}`} className="text-primary hover:underline">{customer.email}</a></div>
                            <div><strong className="font-medium text-slate-600 dark:text-slate-400 block">Phone</strong> <a href={`tel:${customer.phone}`} className="text-primary hover:underline">{customer.phone}</a></div>
                            {customer.alternate_mobile && (
                                <div>
                                    <strong className="font-medium text-slate-600 dark:text-slate-400 block">Alternate Mobile</strong>
                                    <span className="flex items-center gap-1.5">
                                        <a href={`tel:${customer.alternate_mobile}`} className="text-primary hover:underline">{customer.alternate_mobile}</a>
                                        {customer.alternate_is_whatsapp && (
                                            <a href={`https://wa.me/${customer.alternate_mobile.replace(/[^0-9]/g, '').length === 10 ? '91' + customer.alternate_mobile.replace(/[^0-9]/g, '') : customer.alternate_mobile.replace(/[^0-9]/g, '')}`} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-[10px] font-bold bg-green-50 dark:bg-green-900/30 text-green-700 dark:text-green-300 border border-green-200 dark:border-green-800 px-1.5 py-0.5 rounded-full hover:bg-green-100 transition-colors">
                            ...
                            <div><strong className="font-medium text-slate-600 dark:text-slate-400 block">PAN Number</strong> {customer.pan_number || 'N/A'}</div>
                            <div><strong className="font-medium text-slate-600 dark:text-slate-400 block">Aadhaar Card</strong> {customer.aadhar_number || 'N/A'}</div>
                            <div><strong className="font-medium text-slate-600 dark:text-slate-400 block">Service Name</strong> {customer.service_name}</div>
                            <div><strong className="font-medium text-slate-600 dark:text-slate-400 block">Lead Source</strong> {customer.lead_source}</div>
                            {customer.lead_source === 'Customer Referral' && customer.referred_by_customer_id && (
                                <div>
                                    <strong className="font-medium text-slate-600 dark:text-slate-400 block">Referring Customer</strong>
                                    <span>
                                        {(() => {
                            ...
                            {customer.lead_source === 'Employer Referral' && customer.referred_by_employee_id && (
                                <div>
                                    <strong className="font-medium text-slate-600 dark:text-slate-400 block">Referring Employee</strong>
                                    <span>
                                        {(() => {
                            ...
                            <div><strong className="font-medium text-slate-600 dark:text-slate-400 block">Business Category</strong> {customer.business_category || 'Other'}</div>
                            <div><strong className="font-medium text-slate-600 dark:text-slate-400 block">Industry Type</strong> {customer.industry_type || 'Other'}</div>
                            <div>
                                <strong className="font-medium text-slate-600 dark:text-slate-400 block">PAN Card</strong>
                                <DocStatusChip doc={panDoc} />
                            </div>
                            <div>
                                <strong className="font-medium text-slate-600 dark:text-slate-400 block">Date of Birth</strong>
                                <span>{formatDob(customer.date_of_birth)}</span>
                            </div>
                            <div className="sm:col-span-2"><strong className="font-medium text-slate-600 dark:text-slate-400 block">Residential Address</strong> {customer.residential_address || 'N/A'}</div>
                            <div className="sm:col-span-2"><strong className="font-medium text-slate-600 dark:text-slate-400 block">Business Address</strong> {customer.business_address || 'N/A'}</div>
                        </CardContent>
                    </Card>
                    ...
                    <Card>
                        ...
                        <CardContent>
                            <div className="flex justify-between text-sm mb-1">
                                ...
                                <span className="text-slate-500 dark:text-slate-400">Total: ₹{customer.payment_details.total_payment?.toLocaleString('en-IN')}</span>
                            </div>
                    ...
                    <Card>
                        ...
                        <CardContent>
                            {(allPayments && allPayments.length > 0) ? (
                                <>
                                    <div className="flex items-center justify-between mb-3 border-b border-slate-200 dark:border-white/5 pb-3">
                                        <div className="flex items-center gap-2">
                                            ...
                                            />
                                            <label htmlFor="select-all-payments" className="text-sm font-medium text-slate-700 dark:text-slate-300">Select All</label>
                                        </div>
                                    ...
                                    <ul className="space-y-3 max-h-60 overflow-y-auto">
                                        {allPayments.map(payment => (
                                            <li key={payment.id} className="p-3 bg-slate-50 dark:bg-slate-950/40 rounded-lg flex justify-between items-center group">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div>
                                                        ...
                                                        <div className="text-xs text-slate-500 dark:text-slate-400 flex items-center flex-wrap gap-2 mt-0.5">
                                                            <span>{new Date(payment.date).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })} via {payment.method}</span>
                                                            {payment.service_name && (
                                                                <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-medium bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 border border-blue-100 dark:border-blue-800">
                                                                    {payment.service_name}
                                                                </span>
                                                            )}
                                                        </div>
                                ...
                                <p className="text-sm text-slate-500 dark:text-slate-400 italic">No payments recorded for this customer.</p>
                            )}
                        </CardContent>
                    </Card>
                    <Card>
                            ...
                            {customer.uploaded_documents && customer.uploaded_documents.length > 0 ? (
                                <ul className="space-y-2">
                                    {customer.uploaded_documents.map(doc => (
                                        <li key={doc.id} className="text-sm flex justify-between items-center group bg-slate-50 dark:bg-slate-950/40 p-2 rounded-md">
                                            <div className="flex-1 overflow-hidden">
                                                <p className="font-medium truncate text-slate-800 dark:text-slate-200" title={doc.name}>{doc.type}</p>
                                                <p className="text-xs text-slate-500 dark:text-slate-400 truncate">{doc.name}</p>
                                            </div>
                                ...
                                <p className="text-sm text-slate-500 dark:text-slate-400 italic">No documents found.</p>
                            )}
                        </CardContent>
                    </Card>
                </div>
            ...
            <Dialog isOpen={isEditOpen} onClose={() => setIsEditOpen(false)} title="Edit Customer Profile" maxWidth="max-w-xl">
                <form onSubmit={handleEditSubmit} className="space-y-4 pt-4">
                    <div>
                        ...
                        <input type="text" value={editName} onChange={(e) => setEditName(e.target.value)} required className="w-full px-3 py-2 border border-slate-200 dark:border-white/10 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary text-sm bg-background dark:bg-slate-950 text-foreground dark:text-white" />
                    </div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1">Email *</label>
                            <input type="email" value={editEmail} onChange={(e) => setEditEmail(e.target.value)} required className="w-full px-3 py-2 border border-slate-200 dark:border-white/10 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary text-sm bg-background dark:bg-slate-950 text-foreground dark:text-white" />
                        </div>
                        <div>
                            <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1">Phone *</label>
                            <input type="text" value={editPhone} onChange={(e) => setEditPhone(e.target.value)} required className="w-full px-3 py-2 border border-slate-200 dark:border-white/10 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary text-sm bg-background dark:bg-slate-950 text-foreground dark:text-white" />
                        </div>
                    </div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1">Business Name *</label>
                            <input type="text" value={editBusinessName} onChange={(e) => setEditBusinessName(e.target.value)} required className="w-full px-3 py-2 border border-slate-200 dark:border-white/10 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary text-sm bg-background dark:bg-slate-950 text-foreground dark:text-white" />
                        </div>
                        <div>
                            <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1">Date of Birth</label>
                            <input type="date" value={editDob} onChange={(e) => setEditDob(e.target.value)} className="w-full px-3 py-2 border border-slate-200 dark:border-white/10 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary text-sm bg-background dark:bg-slate-950 text-foreground dark:text-white" />
                        </div>
                    </div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1">Business Category *</label>
                            <SearchableSelect
                        ...
                        <div>
                            <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1">Industry Type *</label>
                            <SearchableSelect
                    ...
                    {editLeadSource === 'Customer Referral' && (
                        <div>
                            <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1">Referring Customer *</label>
                            <SearchableSelect
                    ...
                    {editLeadSource === 'Employer Referral' && (
                        <div>
                            <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1">Referring Employee *</label>
                            <SearchableSelect
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1">PAN Number</label>
                            <input type="text" value={editPanNumber} onChange={(e) => setEditPanNumber(e.target.value)} className="w-full px-3 py-2 border border-slate-200 dark:border-white/10 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary text-sm bg-background dark:bg-slate-950 text-foreground dark:text-white" />
                        </div>
                        <div>
                            <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1">Aadhar Number</label>
                            <input type="text" value={editAadharNumber} onChange={(e) => setEditAadharNumber(e.target.value)} className="w-full px-3 py-2 border border-slate-200 dark:border-white/10 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary text-sm bg-background dark:bg-slate-950 text-foreground dark:text-white" />
                        </div>
                    </div>
                    <div className="border-t border-slate-200 dark:border-white/10 pt-4">
                        <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1">Alternate Mobile Number</label>
                        <div className="flex items-start gap-3">
                            <input type="tel" value={editAlternateMobile} onChange={(e) => setEditAlternateMobile(e.target.value)} placeholder="e.g., 9876543211" className="flex-1 px-3 py-2 border border-slate-200 dark:border-white/10 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary text-sm bg-background dark:bg-slate-950 text-foreground dark:text-white" />
                            <div
                                className={`flex items-center gap-2 px-3 py-2 rounded-lg border cursor-pointer transition-all select-none ${editAlternateIsWhatsapp ? 'bg-green-50 dark:bg-green-900/30 border-green-400 text-green-700 dark:text-green-300' : 'bg-slate-50 dark:bg-slate-950/40 border-slate-300 dark:border-white/10 text-slate-600 dark:text-slate-400'}`}
                        ...
                        {editAlternateMobile && (
                            <p className={`text-xs mt-1.5 font-medium ${editAlternateIsWhatsapp ? 'text-green-600 dark:text-green-400' : 'text-slate-500 dark:text-slate-400'}`}>
                    ...
                    <div>
                        ...
                        <textarea value={editResidentialAddress} onChange={(e) => setEditResidentialAddress(e.target.value)} className="w-full px-3 py-2 border border-slate-200 dark:border-white/10 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary text-sm bg-background dark:bg-slate-950 text-foreground dark:text-white h-20" />
                    </div>
                    <div>
                        <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1">Business Address</label>
                        <textarea value={editBusinessAddress} onChange={(e) => setEditBusinessAddress(e.target.value)} className="w-full px-3 py-2 border border-slate-200 dark:border-white/10 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary text-sm bg-background dark:bg-slate-950 text-foreground dark:text-white h-20" />
                    </div>
                    <div className="flex justify-end gap-2 border-t border-slate-200 dark:border-white/10 pt-4">
                        <Button type="button" variant="outline" onClick={() => setIsEditOpen(false)}>Cancel</Button>
                        <Button type="submit" className="bg-primary text-primary-foreground hover:opacity-90">Save Changes</Button>
                    </div>
                </form>
            </Dialog>
        </div>
