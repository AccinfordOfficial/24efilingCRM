    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <div>
                    <h2 className="text-xl font-bold text-white">Document Template Generator</h2>
                    <p className="text-xs text-slate-400">Manage standard engagement letters, NOCs, power of attorney forms, and legal agreements.</p>
                </div>
            ...
            <Card className="bg-slate-900/60 border-white/10">
                <CardContent className="p-4">
                    {isLoading ? (
                        <div className="py-8 text-center text-slate-400 text-sm">Loading legal document templates...</div>
                    ) : templates.length === 0 ? (
                        <div className="py-12 text-center text-slate-400">
                            <FileCheck className="h-8 w-8 mx-auto mb-2 text-slate-500" />
                            <p className="font-semibold text-slate-300">No Legal Templates Created</p>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {templates.map((tpl) => (
                                <div key={tpl.id} className="p-4 bg-slate-950/40 border border-white/5 rounded-lg space-y-3">
                                    <div className="flex justify-between items-start">
                                        <div>
                                            <h3 className="font-bold text-white text-sm">{tpl.name}</h3>
                                            <Badge variant="outline" className="text-[10px] uppercase bg-blue-500/10 text-blue-400 border-blue-500/20 mt-1">
                                    ...
                                    <div className="text-xs text-slate-400 line-clamp-2 bg-slate-900/40 p-2.5 rounded border border-white/5 font-mono">
            ...
            {previewTemplate && (
                <Dialog isOpen={!!previewTemplate} onClose={() => setPreviewTemplate(null)} title={`Preview: ${previewTemplate.name}`}>
                    <div className="space-y-4">
                        <div className="p-4 bg-white text-slate-900 rounded-lg text-sm font-serif leading-relaxed"
                            dangerouslySetInnerHTML={{ __html: renderPreviewHtml(previewTemplate.body_html) }} />
                        <div className="flex justify-end">
                            <Button onClick={() => setPreviewTemplate(null)} className="bg-[#1c398e] text-white">Close</Button>
                        </div>
                    </div>
                </Dialog>
            )}
